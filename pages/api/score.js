...
PROBLEM (0-100): how clearly this post's MAIN TOPIC matches one of the specific problems above.
- 80-100: explicitly experiencing this exact problem as the main topic of the post
- 40-79: related pain or adjacent problem, or the problem is present but not the main topic
- 0-39: no real match, or only mentioned in passing / as background context
...
Posts: ${JSON.stringify(input)}`,
  ...
  const allResults = scored
    ...
    .map(s => {
      ...
      const final = Math.round(problem * WEIGHTS.problem + intent * WEIGHTS.intent + icp * WEIGHTS.icp)
