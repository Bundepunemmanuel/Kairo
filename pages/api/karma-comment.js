...
// ─── Thread picking ───────────────────────────────────────────────────────
// Content-shape only. Velocity/vote scoring was removed — Reddit's RSS
// feed never carries upvote/comment counts, so that signal was always
// inert dead code (see index.ts's toKarmaCandidates for the same note).
//
// FRESH_WINDOW_MINUTES matches the cron interval (hourly, see index.ts's
// header comment). Widened from 15 to 55 — leaves a small buffer before
// the next hourly run so candidates don't age out right at the boundary,
// while still using the full window instead of a nearly-empty 15-minute
// slice of it.
const FRESH_WINDOW_MINUTES = 55

async function pickByContentShape(candidates) {
  const fresh = candidates.filter(c => c.ageMinutes <= FRESH_WINDOW_MINUTES)
  if (!fresh.length) return []

  const input = fresh.map((c, i) => ({
    i,
    title: c.title,
  ...
  const messages = [{
    role: 'user',
    content: `Which of these Reddit posts (if any) look like they'd get noticeably more engagement than a typical post in their subreddit — not necessarily viral, just clearly above average.

The pattern to look for: invites a personal story or opinion (not a factual answer), relatable without needing much context, emotionally light (nostalgia, minor confessions, relatable annoyances — not genuinely divisive or rule-breaking), and short/punchy phrasing rather than long or hedge-y. Use the body text too, not just the title — some posts have a flat title but a genuinely engaging body.

Posts: ${JSON.stringify(input)}

Reply with ONLY a JSON object: {"picks": [<index>, <index>, ...]} ranked best first, up to 3 posts that qualify. Use an empty array {"picks": []} if truly none of them have that shape. No explanation.`,
  }]
  ...
  try {
    ...
    return parsed.picks
      .filter(i => typeof i === 'number' && fresh[i])
      .map(i => fresh[i])
  } catch {
    return []
  }
...
// thing keeping this infrequent is that content-shape is genuinely
...
export default async function handler(req, res) {
  ...
  try {
    const ranked = await pickByContentShape(candidates)

    if (!ranked.length) {
      console.log('[karma] no candidate cleared content-shape this run')
      return res.status(200).json({ picked: null })
    }

    console.log(`[karma] ${ranked.length} candidate(s) cleared content-shape`)

    // Try each ranked pick in order — drafting can still fail (all three
    // model calls empty, or the draft gets rejected) for an individual
    // post without meaning the whole run should come up empty.
    for (const candidate of ranked) {
      const draftComment = await draftKarmaComment(candidate)
      if (draftComment) {
        console.log(`[karma] picked "${candidate.title.slice(0, 60)}" | draft: "${draftComment}"`)
        return res.status(200).json({
          picked: { ...candidate, draftComment, pickedVia: 'content_shape' },
        })
      }
      console.log(`[karma] drafting failed for "${candidate.title.slice(0, 60)}", trying next pick`)
    }

    console.log('[karma] drafting failed for every ranked candidate this run')
    return res.status(200).json({ picked: null })
  } catch (err) {
