...
async function cerebrasReply(messages, _isRetry = false) {
  try {
    ...
    if (!raw && !_isRetry) {
      console.log('[reply:cerebras] empty content on success response — retrying once')
  ...
  try {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages,
        max_tokens: 2000, // reasoning + answer both need to fit
        temperature: 0.75,
...
async function generateWithFallback(messages) {
  ...
  if (!reply || reply.length < 15 || isHardReject(reply)) {
    console.log(reply ? '[reply] Cerebras response was a refusal — trying Groq' : '[reply] Cerebras empty — trying Groq')
    reply = await groqReply(messages)
    reply = reply.replace(/<think>[\s\S]*?<\/think>/g, '').trim()
  }

  if (!reply || reply.length < 15 || isHardReject(reply)) {
    console.log(reply ? '[reply] Groq response was a refusal — trying Nemotron (final)' : '[reply] Groq empty — trying Nemotron (final)')
