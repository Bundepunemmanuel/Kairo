...
async function cerebrasReply(messages, _isRetry = false) {
  try {
    const res = await fetch('https://api.cerebras.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'gpt-oss-120b', messages, max_tokens: 500, temperature: 0.75 }),
    })
    const data = await res.json()
...
function humanizeText(text) {
  ...
  // Em dashes are one of the most commonly-flagged AI-writing tells.
  // Handle contextually rather than blind-replacing every instance:
  ...
  // Collapse any double punctuation/spacing the dash handling might have
...
- Avoid generic phrases like "I understand," "it sounds like," "happy to help"`,
    ...
    if (reply) reply = humanizeText(reply)
