...
async function groqReply(messages) {
  try {
    ...
    console.log('[reply:groq] finish_reason:', data.choices?.[0]?.finish_reason, '| content length:', raw.length)
...
async function cerebrasReply(messages, _isRetry = false) {
  try {
    ...
    const raw = data.choices?.[0]?.message?.content || ''
    console.log('[reply:cerebras] finish_reason:', data.choices?.[0]?.finish_reason, '| content length:', raw.length)
...
async function nemotronReply(messages) {
  try {
    ...
    console.log('[reply:nemotron] finish_reason:', data.choices?.[0]?.finish_reason, '| content length:', raw.length)
