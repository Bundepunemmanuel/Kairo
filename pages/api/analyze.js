...
If website content is empty, infer all fields from the URL and domain name. Return only the JSON object, nothing else.`,
    ...
    // Always return something — never 500 on empty/malformed response
    if (!analysis) {
      console.log('[analyze] groq returned empty/unparseable — using URL-based fallback')
      const domain = url.replace(/https?:\/\//, '').split('/')[0].replace('www.', '')
      analysis = {
        name: domain,
        description: 'A product at ' + domain,
        category: 'software tool',
        specificProblems: ['struggling to find customers', 'need more users', 'distribution is hard'],
        problemKeywords: ['customers', 'users', 'distribution', 'growth', 'marketing'],
        falsePositiveSignals: ['learning to code', 'tutorial', 'beginner'],
        competitors: [],
        subreddits: ['SaaS', 'indiehackers', 'entrepreneur', 'smallbusiness', 'startups', 'freelance'],
      }
    }

    // Ensure all arrays exist
    if (!Array.isArray(analysis.subreddits) || !analysis.subreddits.length) analysis.subreddits = ['SaaS', 'indiehackers', 'entrepreneur', 'smallbusiness', 'startups', 'freelance']
    ...
    console.log('[analyze] returning:', analysis.name, '| subreddits:', analysis.subreddits.join(', '))
    return res.status(200).json({ analysis })
  } catch (err) {
    console.error('[analyze] fatal error:', err.message)
    // Even on fatal error, return a usable fallback
    const domain = url.replace(/https?:\/\//, '').split('/')[0].replace('www.', '')
    return res.status(200).json({
      analysis: {
        ...
        description: 'A product at ' + domain,
        category: 'software tool',
        specificProblems: ['struggling to find customers', 'need more users'],
        problemKeywords: ['customers', 'users', 'distribution', 'growth'],
        falsePositiveSignals: ['learning to code', 'tutorial'],
        competitors: [],
        subreddits: ['SaaS', 'indiehackers', 'entrepreneur', 'smallbusiness', 'startups', 'freelance'],
      }
    })
