...
async function fetchSubredditFromBrowser(subreddit, keywords) {
  try {
    ...
    const res = await fetch(`/api/reddit?sub=${encodeURIComponent(subreddit)}&sort=new${kw}`, { signal: AbortSignal.timeout(10000) })
    if (!res.ok) return []
...
export default function SharePage({ token, initialStatus, initialData }) {
  ...
  const handleNotifySubmit = async () => {
    ...
    try {
      ...
      // Same public homepage ticker as the main onboarding scan — this is
      // just as much "someone scanned their product" as that flow is.
