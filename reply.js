...
// Cerebras llama3.1-70b (primary, 1M tokens/day) → Groq openai/gpt-oss-120b (fallback) → Nemotron 3 Ultra via OpenRouter free tier (final fallback, no retry)
...
async function cerebrasReply(messages, _isRetry = false) {
  try {
    const res = await fetch('https://api.cerebras.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'llama3.1-70b', messages, max_tokens: 500, temperature: 0.75 }),
    })
    const data = await res.json()
