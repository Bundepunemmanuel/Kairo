...
// Scoring: Cerebras gpt-oss-120b (primary, 1M tokens/day) → Nemotron 3 Ultra via OpenRouter free tier (2nd, no shared quota risk with Cerebras) → Groq openai/gpt-oss-120b (final fallback — reordered last after its 200K TPD quota ran out mid-day on Aug 20 and wasted the time budget every request needed for a real fallback to complete)
...
Posts: ${JSON.stringify(input)}`,
  ...
  // every user).
  //
  // Nemotron is second, not last, as of the Aug 20 outage: Groq's 200K
  // TPD budget was exhausted mid-day, and every request during that
  // window burned 15-30s on a doomed rate-limit retry before ever
  // reaching the real final fallback — which then had almost no time
  // left before Vercel's 60s hard timeout killed the whole function.
  // Nemotron has no shared daily-token risk with Cerebras's own outages,
  // so it gets tried while there's still real time budget left, and
  // Groq — the one actually prone to running dry — is now the true
  // last resort instead of a wasted middle step.
  console.log('[score] trying Cerebras (primary)')
  let raw = await cerebras(messages, 1500, 0.1)

  if (!raw) {
    console.log('[score] Cerebras empty — falling back to Nemotron')
    raw = await nemotron(messages, 2500, 0.1)
  }

  if (!raw) {
    console.log('[score] Nemotron empty — falling back to Groq (final)')
