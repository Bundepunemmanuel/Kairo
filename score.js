...
export default async function handler(req, res) {
  ...
  try {
    ...
    const leads = await Promise.all(
      top5.map(async ({ post, score, signalType, specificProblem, reason }) => {
        if (!post) return null

        // Check comments for stronger signal
        const postComments = commentsMap[post.id] || []
        let bestComment = null
        if (postComments.length > 0) {
          try { bestComment = await scoreComments(post, postComments, analysis) } catch {}
        }

        const useComment = bestComment && bestComment.score > score
        const finalScore = useComment ? bestComment.score : score
        const finalSignalType = useComment ? bestComment.signalType : signalType
        const finalProblem = useComment ? bestComment.specificProblem : specificProblem
        const targetComment = useComment ? bestComment : null

        let draftReply = ''
        try { draftReply = await generateReply(post, analysis, finalSignalType, finalProblem, targetComment) } catch {}
        ...
        console.log(`[score] lead: "${post.title.slice(0, 60)}" score=${finalScore}`)
