...
${JSON.stringify(batchInput)}`,
    ...
    try {
      ...
      console.log(`[score] batch ${batchIdx}: ${scored.length} leads found, indexes: ${scored.map(s=>s.index).join(',')}, batchSize: ${batch.length}`)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[score] top5 post check:', top5.map(s => s.post ? s.post.title?.slice(0,40) : 'NULL').join(' | '))
