// score.js — Two-pass Groq scoring pipeline
//
// PASS 1 (pain filter): Fast cheap call — "does this post describe ANY unsolved problem?"
//   No product context. Just finds posts where someone is struggling with something.
//   Runs on all posts in parallel batches of 30. Costs ~1 Groq call per 30 posts.
//
// PASS 2 (product match): Strict call — "does this specific problem match our product?"
//   Only runs on posts that passed Pass 1. Full product context, hard rejection rules.
//   Returns scores 5-10 with specific problem + reason.
//
// PASS 3 (comments): For each qualifying post, scores comments for stronger signals.
// PASS 4 (reply): Generates a grounded, specific reply for each final lead.
...
// ─── PASS 1: Pain filter ──────────────────────────────────────────────────
// Runs on all posts. No product context — just finds posts where someone
// is actively struggling with an unsolved problem.
// Returns indexes of posts that contain real pain signals.

async function filterPainfulPosts(posts) {
  if (!posts.length) return []

  // Process in batches of 30 in parallel
  const BATCH_SIZE = 30
  const batches = []
  ...
  const batchResults = await Promise.all(batches.map(async batch => {
    const raw = await callGroq([
      {
        role: 'system',
        content: 'You are a pain signal detector. Return only a valid JSON array. No markdown.',
      },
      {
        role: 'user',
        content: `Look at these Reddit posts. Return ONLY the indexes of posts where someone is describing an ACTIVE, UNSOLVED problem or frustration.

INCLUDE if the post:
- Describes a painful situation they are currently in
- Asks for help with something they are struggling with right now
- Expresses frustration with a current tool or process
- Is looking for recommendations or alternatives
- Describes a workflow that is not working

EXCLUDE if the post:
- Is a success story or lesson learned (already solved)
- Is general discussion, news, or opinion with no personal problem
- Is asking how to START something (beginner/learning)
- Is someone sharing advice or tips (they solved it)
- Has no clear problem statement at all

Return ONLY a JSON array of indexes: [0, 3, 7, 12]
...
${JSON.stringify(batch)}`,
    ...
    try {
      ...
      if (!match) return []
      return JSON.parse(match[0]).filter(i => typeof i === 'number')
    } catch {
      return []
    }
  }))
...
export default async function handler(req, res) {
  ...
  try {
    // ── Pass 1: Find posts with any pain signal (no product context)
    let painIndexes = []
    try {
      painIndexes = await filterPainfulPosts(posts)
    } catch {
      // If Pass 1 fails, let everything through to Pass 2
      painIndexes = posts.map((_, i) => i)
    }

    if (!painIndexes.length) {
      return res.status(200).json({ leads: [] })
    }

    const painfulPosts = painIndexes.map(i => posts[i]).filter(Boolean)

    // ── Pass 2: Match painful posts against the specific product
    let scores = []
    try {
    ...
    const leads = await Promise.all(
      top5.map(async scored => {
        ...
        if (expiresIn <= 0) return null // Expired — skip
