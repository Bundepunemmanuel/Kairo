...
export default function Dashboard() {
  ...
  const [confirmBox, setConfirmBox] = useState(null) // { leadId, text } — editable "what did you actually send" step
  const [confirmSaving, setConfirmSaving] = useState(false)
  const [theirReplyInput, setTheirReplyInput] = useState({}) // { [leadId]: string }
  const [followupLoading, setFollowupLoading] = useState({}) // { [leadId]: bool }
  ...
  const handleOpenConfirmBox = (lead) => {
    ...
    setConfirmBox({ leadId: lead.id, text: draftText, isFollowup: false })
  }

  const handleOpenFollowupConfirmBox = (lead, draftText) => {
    setConfirmBox({ leadId: lead.id, text: draftText, isFollowup: true })
  }
  ...
  // User pastes what the thread owner replied with — AI judges whether to
  // keep going and, if so, drafts the next reply (which itself still
  // needs the same confirm-edit step before being marked sent).
  const handleSubmitTheirReply = async (lead) => {
    ...
    try {
      ...
      // Persist their reply (and the closed decision, if applicable) right
      // away — this is a factual record regardless of what happens next.
      await supabase
        .from('leads')
      ...
      if (status === 'closed') {
        ...
        // Update local copy so the confirm box + conversation history
        // reflect their new message immediately, then open the confirm
        // box with the AI's suggested next reply.
      ...
      console.log('[dashboard] follow-up error:', e.message)
    } finally {
      ...
      <div className="dash-page">
        ...
        {neverScanned ? (
          ...
          <>
            ...
            {convoLeads.length > 0 && (
              <div className="dash-convo-section">
                <h2 className="dash-section-title">💬 Active conversations</h2>
                <p className="dash-section-sub">Threads still in progress. Kairo will let you know when one wraps up.</p>

                {convoLeads.map(lead => {
                  const convo = Array.isArray(lead.conversation) ? lead.conversation : []
                  const isFollowupLoading = followupLoading[lead.id]
                  return (
                    <div key={lead.id} className="dash-lead-card dash-convo-card">
                      <h3 className="dash-lead-title">{lead.title}</h3>
                      <span className="dash-lead-sub">r/{lead.subreddit}</span>
