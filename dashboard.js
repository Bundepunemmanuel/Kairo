...
export default function Dashboard() {
  ...
  const handleDelete = async (leadId) => {
    // Soft delete — moves to archive, does NOT affect daily quota
    setActiveLeads(prev => prev.filter(l => l.id !== leadId))
    try {
      await supabase.from('leads').update({ deleted: true }).eq('id', leadId)
      await loadData()
    } catch (e) {
      console.log('[dashboard] delete error:', e.message)
    }
  }
  ...
  const handleMarkReplied = async (leadId) => {
    setActiveLeads(prev => prev.filter(l => l.id !== leadId))
    try {
      await supabase.from('leads').update({ replied: true }).eq('id', leadId)
      await loadData()
    } catch (e) {
      console.log('[dashboard] mark replied error:', e.message)
    }
  }
