...
async function gemini(messages, maxTokens, temperature, _isRetry = false) {
  try {
    const res = await fetch('https://generativelanguage.googleapis.com/v1beta/openai/chat/completions', {
      ...
      // Without this, a hung Gemini response runs until Vercel's own 60s
      // hard-kill instead of failing fast enough to let Groq/Nemotron get
      // a real shot — exactly what happened in production (see the "jina
      // content length... trying Gemini... Task timed out after 60
      // seconds" log with nothing in between).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (jinaRes.ok) {
      ...
      // Reject HTML error pages — Jina sometimes returns these on failure
...
- For scheduling/calendar tools: freelance, consulting, smallbusiness, Entrepreneur, remotework
- For payment/billing tools: ecommerce, smallbusiness, entrepreneur, SaaS, webdev
- For AI image/content tools: StableDiffusion, midjourney, artificial, MachineLearning, graphic_design, ecommerce
- For social media tools: socialmedia, marketing, smallbusiness, content_marketing, Entrepreneur
- For email marketing tools: marketing, emailmarketing, smallbusiness, entrepreneur, ecommerce
- For project management tools: projectmanagement, devops, startups, SaaS, smallbusiness
- For design tools (Figma, Framer, Webflow, Canva): graphic_design, web_design, UI_Design, freelance, webdev, marketing
- NEVER pick tiny topic-named subreddits or learning subreddits
- ALWAYS pick based on where the PROBLEM surfaces — not where the product category name lives
